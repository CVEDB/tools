<template>
  <q-layout view="lHh Lpr lFf">
    <q-header elevated>
      <q-toolbar>
        <q-toolbar-title>
          <router-link to="/home" class="text-white" style="text-decoration: none;">CVEDB Web</router-link>
        </q-toolbar-title>
...
export default defineComponent({
  ...
  setup() {
    ...
    function search() {
      ...
      if (searchField.value.match(/^CVEDB-\d{4}-\d{4,}$/) || searchField.value.match(/^UVI-\d{4}-\d{4,}$/)) {
        $router.push({ path: `/${searchField.value}` })
      } else {
        window.open(
          `https://github.com/CVEDB/database/search?q=${searchField.value}`,
          '_blank'
        )
      }
    }
    ...
    function openGithubRepo() {
      window.open('https://github.com/CVEDB/tools/tree/main/web#web', '_blank')
    }
    ...
    const loginURL = `https://github.com/login/oauth/authorize?client_id=${process.env.CVEDB_GITHUB_KEY}&scope=public_repo`
    ...
    const username = ref('')
    username.value = $q.cookies.get('CVEDB-USERNAME')
