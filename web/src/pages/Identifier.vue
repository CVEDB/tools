            ...
            >
              <q-card style="background: #2d2d2d;">
                <q-card-section style="overflow: auto; max-height: 80vh;">
                  <a
                    :href="`https://api.cvedb.id/${identifier}`"
                    target="_blank"
                    v-if="jsonBlob"
                    style="color: white;"
                  >
                    https://api.cvedb.id/{{ identifier }}
                  </a>
...
import { cvedbApi } from 'boot/axios'
  ...
  setup() {
    ...
    function updateJsonBlob() {
      ...
      if (isValidIdentifier(identifier.value)) {
        cvedbApi.get(`/${identifier.value}`).then(
          (response) => {
    ...
    function isValidIdentifier(value) {
      return (value.match(/^CVEDB-\d{4}-\d{4,}$/))
    }
    ...
    function editCVEDB() {
      ...
      // const repo = 'https://github.com/CVEDB/database'
      // const branch = 'main'
