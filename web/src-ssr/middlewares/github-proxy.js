...
const githubClientID = process.env.CVEDB_GITHUB_KEY
const githubClientSecret = process.env.CVEDB_GITHUB_SECRET
    ...
    try {
      ...
      if (Array.isArray(existingPullRequests) && existingPullRequests.length > 0) {
        ...
        const pullResponse = await octokit.request('POST /repos/{owner}/{repo}/pulls', {
          ...
          repo: 'database',
          title: title,
          body: 'This pull request was created using https://cvedb.id',
          head: branchHead,
          base: 'main',
