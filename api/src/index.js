...
const host = 'https://raw.githubusercontent.com/CVEDB/database/main'
...
*/
router.get("/", () => {
  return Response.redirect('https://cvedb.id', 307);
})
...
router.get("/:id", async ({ params }) => {
  ...
  if (!(identifier.match(/^CVEDB-\d{4}-\d{4,}$/) || identifier.match(/^UVI-\d{4}-\d{4,}$/))) {
    // TODO: Support other ID formats. Also support stripping `.json` if included
    return new Response(
      'Invalid CVEDB format! Expected something like CVEDB-2021-1002352.',
      { status: 400 }
    )
  }
